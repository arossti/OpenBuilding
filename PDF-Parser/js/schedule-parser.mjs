...
export function extractTable(textItems, options) {
  ...
  return rows.map(function(row) {
    ...
    var cells = [], currentCell = row[0].str;
...
export function findRoomSchedule(pageNum) {
  return Loader.getTextContent(pageNum).then(function(textItems) {
    ...
    var headerIdx = -1, roomCol = -1, areaCol = -1, numCol = -1;

    for (var r = 0; r < table.length; r++) {
      for (var c = 0; c < table[r].length; c++) {
        var cell = table[r][c].toLowerCase();
        if (cell === "room" || cell === "room name") { roomCol = c; headerIdx = r; }
        if (cell === "area" || /area\s*\(m/.test(cell)) { areaCol = c; }
        if (cell === "#" || cell === "no." || cell === "num") { numCol = c; }
      }
