...
### Status as of 2026-04-19 (session 5 — Phase 3 parity-validated with BEAM gSheet; F&S calc pivoted to BfCA materials DB)

- **Session 5 on `beamweb-tabs-2`** — 11 commits spanning foundational fixes, parity-validation math fixes, a jurisdiction filter, and an architecture pivot so the calc path reads per-unit GWP directly from `schema/materials/index.json` (the same catalogue BEAM's gSheet itself uses). F&S tab now matches the BEAM workbook to the integer kgCO2e on every parity test case, including rows the BEAM sample project did not populate (METAL PILE / TIMBER PILE / REBAR FOR SLABS).
...
5. **Calc data source**: F&S reads per-unit GWP factors from the BfCA materials DB (`data/schema/materials/index.json`) via [`js/beam/materials-db.mjs`](./PDF-Parser/js/beam/materials-db.mjs). `getMaterial(hash)` returns the catalogue entry (id, beam_id, gwp_kgco2e, functional_unit); `convertQtyToMaterialUnit(...)` handles unit mismatch (m² row + m³ material via group THICKNESS). Same catalogue Database viewer serves. No reverse-engineering from sample-project CSV columns. If a row shows the `–` no-factor marker, its hash didn't resolve in the DB (see Known issues below).
6. **Known issues** — some "BEAM Avg" insulation EPDs have `gwp_kgco2e: null` in the DB (e.g. XPS BEAM Avg `a94mfe`). Sample-project SELECTs them by default but the DB doesn't store the computed average. Fix path (locked): compute the average client-side from the manufacturer-specific entries that share the same material subtype. Separate slice. R-VALUE scaling on SUB-SLAB INSULATION is also deferred — at default R the direct per-m² factor matches BEAM; user-modified R currently uses a linear `configRatio` proxy in `groupConfigRatio`.
7. Phase 4 queue: see §6 — 11 assembly tabs, smallest-first (Windows → Garage). Each follows the F&S template: copy [`footings-slabs-tab.mjs`](./PDF-Parser/js/beam/footings-slabs-tab.mjs), reuse the generic `assembly-csv-parser.mjs` + `materials-db.mjs`, add the tab's groups to `PROJECT_TO_FS_GROUPS` in [`auto-fill.mjs`](./PDF-Parser/js/beam/auto-fill.mjs) where applicable (rename to `PROJECT_TO_ASM_GROUPS` once it covers more than just F&S).
8. Remaining cross-cut items: `Categories.csv` dropdown option lists for Building Type / Construction Type on PROJECT, `js/shared/units.mjs` for the metric/imperial toggle, BEAM-Avg insulation fix (above), R-VALUE formula port.
...
- Active feature branch: `beamweb-tabs-2` on both remotes. Tip as of end of session 5: `8bee3f4`. Contains 11 commits covering the cold-start / action-bar foundational fix, Load Sample mechanism, jurisdiction filter + cascading PROJECT dropdowns, four parity-math fixes, architectural cleanup (prettier / eslint / helper consolidation / 2dp display), TOTAL-config → row-qty bridge for METAL / TIMBER PILE + REBAR groups, and the materials-DB migration so F&S reads GWP factors directly from `schema/materials/index.json` (single source of truth, matches what BEAM gSheet does internally). F&S is parity-validated against the BEAM workbook on every row tested.
...
- **2026-04-19 (session 5)** — F&S parity-validated against BEAM gSheet; calc path rewritten to read factors from the BfCA materials DB. Eleven commits on `beamweb-tabs-2`:
  ...
  - `8bee3f4` — Materials-DB as single source of truth. The sample-derived factor approach (reverse-engineering from the assembly CSV's pre-computed NET column) is deleted. New [`js/beam/materials-db.mjs`](./PDF-Parser/js/beam/materials-db.mjs) loads `schema/materials/index.json` (the same catalogue Database viewer uses). `getMaterial(hash)` returns the full entry; `convertQtyToMaterialUnit(rowQty, rowUnit, dbEntry, group, getValue)` handles unit mismatch (m² row + m³ material via group THICKNESS). Parser simplified: no factors, no cross-share, just structure + sample UI state. `computeRowEmissions({select, qtyInMaterialUnit, pct, gwp})` is now pure math. Parity verified: FOOTINGS Concrete 2501-3000 NRMCA = 2,792 ✓, SLABS Concrete 0-2500 NRMCA = 4,704 ✓, METAL PILE Helical pier @ 10m = 176 ✓, METAL PILE Krinner E140x1600 @ 10m = 442 ✓, METAL PILE AGS IM4516 @ 10m = 412 ✓, TIMBER PILE Wood/SPF @ 12m³ = 757 ✓. `groupConfigRatio` now returns 1 for THICKNESS too (unit converter handles it); R-VALUE left as linear configRatio proxy pending proper port.

  **Known issue (deferred):** some "BEAM Avg" insulation EPDs have `gwp_kgco2e: null` in `schema/materials/index.json` (e.g. XPS BEAM Avg `a94mfe`). The BEAM gSheet computes these averages from the manufacturer EPDs on the fly; the DB doesn't store the computed average. Fix path: compute average client-side from manufacturer entries sharing the same material subtype. Next-session work.

  **Known issue (deferred):** R-VALUE scaling on SUB-SLAB INSULATION uses a linear `configRatio = user_R / default_R` proxy in `groupConfigRatio`. At default R the per-m² factor matches BEAM; user-modified R under- or over-calculates until the BEAM R-value formula (R-per-inch × density × thickness conversion) is properly ported. Next-session work.
