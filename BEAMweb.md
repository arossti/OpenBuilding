...
### Status as of 2026-04-18 (end of session 3 — Phase 1 stubs landed, Phase 2 PROJECT tab live)
...
- **BEAM sheet snapshot + fetch script** (session 3, `6b234b3`). [`schema/scripts/fetch-beam-sheet.mjs`](./schema/scripts/fetch-beam-sheet.mjs) pulls every BEAM tab via the Google Sheets `gviz` CSV endpoint — clean CSV with no padding garbage and zero `#NAME?` sentinels. 22 canonical tab snapshots committed under [`docs/csv files from BEAM/`](./docs/csv%20files%20from%20BEAM/). The BEAM sheet is currently locked down; re-run the script after temporarily unlocking when source material changes. Per-tab mapping (for assembly-tab ports in Phase 3+) now has real data to reference. **Note: §4.4 `#NAME?` importer path stays scaffolded for future tabs — the clean gviz exports didn't exercise it, but xlsx-export paths still might.**
...
4. **Phase 3 — First assembly tab end-to-end**. Candidate: `Footings & Slabs` (smallest clean assembly — 318 canonical rows, single unit m³). Establishes the per-row calc pattern, regression-tests against BEAM workbook output with a canonical project, locks the per-tab module shape. Emission-factor source: **match the BEAM workbook's precomputed NET/GROSS columns first** (session 3 decision — see "parity-then-migrate" below); drive from `schema/materials/` DB once BfCA validates functional parity.
