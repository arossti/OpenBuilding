...
| **P2 — UX scaffold (shipped 2026-04-25)** | 50/50 layout (PDF left, schema-form right), schema-driven form pane with ~24 representative fields across 6 sections, IndexedDB auto-save (`state: "draft"`), Capture button promotes draft → captured, audit-row auto-stamp. Database viewer pending-panel + Trust / Trust + Verify stubs (the verify modal shows JSON; per-field diff lands at P4). Manual entry only — no regex auto-fill yet. | ✓ End-to-end demo confirmed 2026-04-26. Drop EPD → fill form → Capture → switch to Database tab → Trust or Trust + Verify works. |
| **P3 — Field extraction (regex auto-fill)** | Anchor-based regex passes against the field groups in §5. Calibrate against the format families documented in §9.5 (NA / EU / EPD International / NSF). Auto-populate the same form fields the user manually enters today. **Regression target — drop ALL 30 sample EPDs through the flow and confirm Capture produces a populated record per sample**; track failures (zero fields / partial / format-family miss) per sample in a calibration log. The current empty-Capture behavior on insulation EPDs is expected for v1 (manual entry only); P3's job is to make this go away. | At least 80% field coverage on the 30-sample regression set with confidence chips for low-certainty extractions. Failure log committed to repo. |
| **P4 — Match + form pane status** | Run the §6 six-key match against the DB at form-render time. Surface match outcome (`NEW` vs `REFRESH → <existing-id>`) in the form's status banner AND in the pending-panel row in the Database viewer (see [`Database.md`](Database.md) §5.5 (a) — required before stub Trust → real Trust). Side-by-side diff lives in the database viewer, not here. | Form populated with match outcome chip; Database pending-panel row shows the same outcome; Trust on a refresh row is visually distinct from Trust on a new row. |
...
## 7.5. Known issues (P2, captured 2026-04-26)

- **Window resize doesn't reflow the canvas viewer.** On initial load the layout fits PDF + form to the viewport correctly (60/40 flex split). On a subsequent browser-window resize the **form pane reflows fine** (CSS flex picks up the new viewport width), but the **PDF canvas stays at its load-time CSS scale** — the rendered page does not grow into the larger pane, leaving dark padding to the right of the document. Root cause: [`js/canvas-viewer.mjs`](../../js/canvas-viewer.mjs) computes `_scale` from `_wrap.clientWidth/clientHeight` only inside `zoomFit()` (called on file-load and on F-key/Fit-button), with no `window.addEventListener("resize", …)` to re-run it. PDF-Parser shares the same module so the issue is technically present there too — less visible because measure-and-polygon users tend to keep windows steady. Fix is small: add a debounced (~150 ms) `resize` listener inside `canvas-viewer.mjs._bindEvents()` that re-runs `zoomFit()` if `_currentPage > 0`. Defer to next session — Andy flagged 2026-04-26.
