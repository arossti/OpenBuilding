...
- ✅ **Parity B Pass-1 harness** (2026-06-08, §19) — SHIPPED `schema/scripts/csv-pdf-parity.mjs` + first run on Andy's ART Database (renamed from BEAM Database-DUMP 2026-06-08 — see §19.1) + 301-PDF Confirmed folder. Scope authored by Andy 2026-06-08 (§19.1.1 canonical 37-column EPD-extractable list). **Current headline: 37.3% aggregate parity** (5,264/14,100 matched cells, 0/671 rows at 100%) post Phase 0a + 1a + 1b + 1c + 2a–2o + W conditional skip. Per-field rank in `docs/workplans/parity-B/parity-summary.md` is the prioritized lift backlog. **Session lift: +27.8pp (+3,928 matches) toward the 50% target.**
...
- ✅ **§19 Phase 2n — J Manufacturer brand/trademark patterns + owner fallback** (2026-06-08) — SHIPPED. **J Manufacturer 34.3% → 37.7%** (+23 matches, +3.4pp on J). Aggregate **37.1% → 37.3%** (+0.2pp / +23 matches). Three fixes: (a) `™®` added to character class — fixed cases like RedBuilt™ LLC where the trademark glyph blocked the suffix-anchored fallback; (b) corporate-suffix list expanded (Pty / PLC / AB / AG / BV / NV / KG / OY / AS / ApS / S.L. / SAS / Group / Industries / Holdings / Company) — catches Australian, UK, German, Dutch, Scandinavian companies; (c) **manufacturer-from-owner fallback** — when no manufacturer pattern fires but `epd.owner` was extracted AND contains a corporate suffix, copy it to manufacturer.name (for product-specific EPDs the Declaration Owner is the manufacturer); (d) brand-mark fallback in head≤30 lines — "<Word> Roofing Products®" / "<Word> Industries" / "by <Brand>" forms. Canonical-30 metadata **73.6% → 74.3%** (+0.7pp) — patterns help calibration too. Narrative manufacturers without corporate suffix (Havelock Wool, Greenfiber) still resist; would need title-block font-size heuristic or full LLM path to catch.
- ✅ **§19 Phase 2o — Curated LLM-knowledge manufacturer dictionary** (2026-06-08) — SHIPPED. **J Manufacturer 37.7% → 38.6%** (+6 matches, +0.9pp on J). Aggregate ratio rounded unchanged but +6 cells (5,258 → 5,264). Added ~150-entry `KNOWN_MFRS` dictionary spanning the major EPD-publishing building-materials manufacturers across wood/timber, insulation, steel, concrete/masonry/cement, roofing, gypsum, membranes/air barriers, windows/glazing, flooring, specialty product categories. Sourced from LLM domain knowledge (Andy's suggestion 2026-06-08), validated against the J parser-empty bucket (RedBuilt, Rothoblaas, Malarkey, Greenfiber, Havelock Wool, CENTRIA, MarinoWARE, Partel, BurntWood, Tarkett, etc.). Detection scoped to head≤80 lines, runs only when prior patterns + owner fallback all failed. Lift smaller than hoped because Phase 2n's owner-fallback already absorbed many obvious cases; the dictionary covers the residual narrative-style mentions. Canonical-30 metadata held at 74.3%, 0 GT failures.
...
> **Status: Pass-1 harness SHIPPED 2026-06-08**; run output committed at `docs/workplans/parity-B/`. **Canonical scope** (Andy 2026-06-08, §19.1.1): 37 EPD-extractable BEAM columns. **Headline: 37.3% aggregate cell parity** (5,258 / 14,100 populated cells matched the parser, post Phase 2n shipped 2026-06-08), **0 of 671 rows at 100%**. Session lift: 9.5% (baseline pre-Phase-0a) → 37.3% = **+27.8pp** via 15 phases (0a + 1a + 1b + 1c + 2a–2n + W conditional skip — see the §0 pending list for per-phase counts). This section is the writeup for what it will take to lift that to **≥50%**, what past issues block the work, and the phased order to attack it.
...
| **Cells matched (numerator)** | **5,258** (was 1,336 pre-Phase-0a; **+3,922 over the session**) |
| **Aggregate parity** | **37.3%** (was 9.5% baseline; **+27.8pp** via 15 phases 0a-2n) |
| Average per-row coverage | 37.9% (was 9.3%) |
...
### 19.2. Where the 37.3% comes from — three contributing layers
...
### 19.4. Phased lift plan — getting from 37.3% to ≥50%

Each phase below states (a) what to do, (b) which in-scope fields it lifts, (c) a **rough** cell-count estimate (not a promise — the harness quantifies after each phase). Aggregate target: from current **5,258 matches → 7,050+** (50% of 14,100). **Phase 3 (true multi-product) puts us at 50%.**
