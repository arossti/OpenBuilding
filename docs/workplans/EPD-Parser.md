...
- ✅ **P2 — UX scaffold** — 60/40 layout (PDF left, schema-driven form right; window-resize listener now in `canvas-viewer.mjs`), 24-field form across 6 sections, IndexedDB auto-save (`state: "draft"`), Capture button promotes draft → captured. Database viewer pending-changes panel + Trust / Trust + Verify stubs ([`Database.md`](Database.md) §4–§5).
...
- ✅ **BEAM ID convention** locked in §5 + §5.5 (6-char `GG####`, never overwrite with PCR or EPD-id).

**Latest measured coverage** (`node schema/scripts/test-epd-extract.mjs`, 2026-04-27 22:35Z):
- 30/30 samples processed, no errors
- **Metadata: 166/330 = 50.3%** (11 fields × 30 samples)
- **Impact totals: 92/300 = 30.7%** (10 indicators × 30 samples)
- Format detection: na=18, unknown=8, eu_ibu=2, nsf=1, epd_international=1

**Phases pending** (ranked by leverage — see §9.5 fix-list for the per-format gaps that drive these):
- ⏳ **P3.2 — Per-format iteration** to push impact coverage past 60%: older BC Wood, Sopra family, EPD-IES filename variant, EU/IBU per-format extractor, PE-NR / PE-R / WDP synonym expansion across NA family.
- ⏳ **P3.3 — Per-stage breakdown** (A1, A2, A3, A1-A3, A4, …, D) — needs column-header parsing for cradle-to-gate vs cradle-to-grave layouts.
- ⏳ **P4 — Match-status surfacing** (`NEW` vs `REFRESH → <id>`) on form banner + Database pending-panel row, per [`Database.md`](Database.md) §5.5(a).
...
- ⏳ **P7 — Coverage hardening** (OCR fallback for the 1 image-only EPD in the calibration set, multi-EPD bulk upload).
