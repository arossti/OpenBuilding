...
- ✅ **Parity B Pass-1 harness** (2026-06-08, §19) — SHIPPED `schema/scripts/csv-pdf-parity.mjs` + first run on Andy's ART Database (renamed from BEAM Database-DUMP 2026-06-08 — see §19.1) + 301-PDF Confirmed folder. Scope authored by Andy 2026-06-08 (§19.1.1 canonical 37-column EPD-extractable list). **Current headline: 36.3% aggregate parity** (5,115/14,100 matched cells, 0/671 rows at 100%) post Phase 0a + 1a + 1b + 1c + 2a–2l + W conditional skip. Per-field rank in `docs/workplans/parity-B/parity-summary.md` is the prioritized lift backlog. **Session lift: +26.8pp (+3,779 matches) toward the 50% target.**
...
> **Status: Pass-1 harness SHIPPED 2026-06-08**; run output committed at `docs/workplans/parity-B/`. **Canonical scope** (Andy 2026-06-08, §19.1.1): 37 EPD-extractable BEAM columns. **Headline: 36.3% aggregate cell parity** (5,115 / 14,100 populated cells matched the parser, post Phase 2l shipped 2026-06-08), **0 of 671 rows at 100%**. Session lift: 9.5% (baseline pre-Phase-0a) → 36.3% = **+26.8pp** via 13 phases (0a + 1a + 1b + 1c + 2a–2l + W conditional skip — see the §0 pending list for per-phase counts). This section is the writeup for what it will take to lift that to **≥50%**, what past issues block the work, and the phased order to attack it.
...
| **Cells matched (numerator)** | **5,115** (was 1,336 pre-Phase-0a; **+3,779 over the session**) |
| **Aggregate parity** | **36.3%** (was 9.5% baseline; **+26.8pp** via 13 phases 0a-2l) |
| Average per-row coverage | 36.8% (was 9.3%) |
...
### 19.2. Where the 36.3% comes from — three contributing layers
...
### 19.4. Phased lift plan — getting from 36.3% to ≥50%

Each phase below states (a) what to do, (b) which in-scope fields it lifts, (c) a **rough** cell-count estimate (not a promise — the harness quantifies after each phase). Aggregate target: from current **5,115 matches → 7,050+** (50% of 14,100). **Phase 3 (multi-product) puts us at 50%.**
