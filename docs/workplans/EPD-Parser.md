...
- ✅ **Parity B Pass-1 harness** (2026-06-08, §19) — SHIPPED `schema/scripts/csv-pdf-parity.mjs` + first run on Jen's April-20 CSV + 301-PDF Confirmed folder. Scope authored by Andy 2026-06-08 (§19.1.1 canonical 37-column EPD-extractable list). **Current headline: 23.6% aggregate parity** (3,328/14,100 matched cells, 0/671 rows at 100%) post Phase 0a + 1a + 1b + 1c + W conditional skip. Per-field rank in `docs/workplans/parity-B/parity-summary.md` is the prioritized lift backlog. **Session lift: +14.1pp (+1,992 matches) toward the 50% target.**
...
> **Status: Pass-1 harness SHIPPED 2026-06-08**; run output committed at `docs/workplans/parity-B/`. **Canonical scope** (Andy 2026-06-08, §19.1.1): 37 EPD-extractable BEAM columns. **Headline: 23.6% aggregate cell parity** (3,328 / 14,100 populated cells matched the parser, post Phase 1c shipped 2026-06-08), **0 of 671 rows at 100%**. Session lift: 9.5% (baseline pre-Phase-0a) → 23.6% = **+14.1pp** via Phase 0a + Phase 1a (AY shape-aware regex, +299) + Phase 1b (six new admin/methodology extractors + manufacturer move, +125) + W conditional skip + Phase 1c (per-column comparison relaxations — substring tolerance for org-name fields, year-prefix for expiry, case-insensitive default — recognizing semantic equivalence of variant BEAM forms, **+1,390 matches**). This section is the writeup for what it will take to lift that to **≥50%**, what past issues block the work, and the phased order to attack it.
...
| **Cells matched (numerator)** | **3,328** (was 1,336 pre-Phase-0a; **+1,992 over the session**) |
| **Aggregate parity** | **23.6%** (was 9.5% baseline; **+14.1pp** via Phase 0a + Phase 1a + 1b + 1c) |
| Average per-row coverage | 23.5% (was 9.3%) |
...
| **0% in-scope fields** | I, K, L, N, O, R, W, AI, AJ, AK, AL, AM, AN, AO, AP, AQ, AR, AS, BK | — | 0 | — | the extractor either doesn't touch the field, or extracts a form that never matches BEAM's stored value — true extractor backlog |

### 19.2. Where the 23.6% comes from — three contributing layers
...
### 19.4. Phased lift plan — getting from 23.6% to ≥50%

Each phase below states (a) what to do, (b) which in-scope fields it lifts, (c) a **rough** cell-count estimate (not a promise — the harness quantifies after each phase). Aggregate target: from current **3,328 matches → 7,050+** (50% of 14,100). **Phase 3 (multi-product) puts us at 50%.**
...
Phase 1 lift estimate: ~+2,200 matches (Phase 1a + 1b + 1c already booked **+1,814**, putting Phase 1 ahead of the original target). Cumulative actual: **23.6%** (3,328 / 14,100). Remaining Phase 1 headroom: AZ vocab expansion to lift the 45% to 70%, G additional date-format coverage, BB Prepared-by (still 3.6% — BEAM and parser often reference genuinely different orgs).
