...
- ✅ **Parity B Pass-1 harness** (2026-06-08, §19) — SHIPPED `schema/scripts/csv-pdf-parity.mjs` + first run on Andy's ART Database (renamed from BEAM Database-DUMP 2026-06-08 — see §19.1) + 301-PDF Confirmed folder. Scope authored by Andy 2026-06-08 (§19.1.1 canonical 37-column EPD-extractable list). **Current headline: 28.9% aggregate parity** (4,075/14,100 matched cells, 0/671 rows at 100%) post Phase 0a + 1a + 1b + 1c + 2a + 2b + 2c + 2d + 2e + 2f + W conditional skip. Per-field rank in `docs/workplans/parity-B/parity-summary.md` is the prioritized lift backlog. **Session lift: +19.4pp (+2,739 matches) toward the 50% target.**
...
> **Status: Pass-1 harness SHIPPED 2026-06-08**; run output committed at `docs/workplans/parity-B/`. **Canonical scope** (Andy 2026-06-08, §19.1.1): 37 EPD-extractable BEAM columns. **Headline: 28.9% aggregate cell parity** (4,075 / 14,100 populated cells matched the parser, post Phase 2f shipped 2026-06-08), **0 of 671 rows at 100%**. Session lift: 9.5% (baseline pre-Phase-0a) → 28.9% = **+19.4pp** via Phase 0a + 1a + 1b + W conditional skip + Phase 1c (per-column comparison relaxations, +1,390) + Phase 2a (BK Service Life / O Markets / K Brand Name, +89) + Phase 2b (BB known-LCA-org + AL/AQ physical + W path norm, +166) + Phase 2c (AT substring + BA known-EPD-owner orgs, +29) + Phase 2d (N Country prose extractor + N/O substring tolerance, +153) + Phase 2e (R declared-unit normalization, +284) + Phase 2f (AK 3-tier extractor + AL labeled-λ patterns + AK/AL numericApprox relaxation, +26). This section is the writeup for what it will take to lift that to **≥50%**, what past issues block the work, and the phased order to attack it.
...
| **Cells matched (numerator)** | **4,075** (was 1,336 pre-Phase-0a; **+2,739 over the session**) |
| **Aggregate parity** | **28.9%** (was 9.5% baseline; **+19.4pp** via Phase 0a + 1a + 1b + 1c + 2a + 2b + 2c + 2d + 2e + 2f) |
| Average per-row coverage | 29.1% (was 9.3%) |
...
### 19.2. Where the 28.9% comes from — three contributing layers
...
### 19.4. Phased lift plan — getting from 28.9% to ≥50%

Each phase below states (a) what to do, (b) which in-scope fields it lifts, (c) a **rough** cell-count estimate (not a promise — the harness quantifies after each phase). Aggregate target: from current **4,075 matches → 7,050+** (50% of 14,100). **Phase 3 (multi-product) puts us at 50%.**
...
Phase 1 lift estimate: ~+2,200 matches (Phase 1a + 1b + 1c already booked **+1,814**, putting Phase 1 ahead of the original target). Cumulative actual: **28.9%** (4,075 / 14,100, including Phase 2a + 2b + 2c + 2d + 2e + 2f). Remaining Phase 1 headroom: AZ vocab expansion (302 rows where parser detected nothing — likely +200-250 matches), G additional date-format coverage.
