...
- ✅ **Parity B Pass-1 harness** (2026-06-08, §19) — SHIPPED `schema/scripts/csv-pdf-parity.mjs` + first run on Andy's ART Database (renamed from BEAM Database-DUMP 2026-06-08 — see §19.1) + 301-PDF Confirmed folder. Scope authored by Andy 2026-06-08 (§19.1.1 canonical 37-column EPD-extractable list). **Current headline: 29.1% aggregate parity** (4,109/14,100 matched cells, 0/671 rows at 100%) post Phase 0a + 1a + 1b + 1c + 2a + 2b + 2c + 2d + 2e + 2f + 2g + W conditional skip. Per-field rank in `docs/workplans/parity-B/parity-summary.md` is the prioritized lift backlog. **Session lift: +19.6pp (+2,773 matches) toward the 50% target.**
...
| **Cells matched (numerator)** | **4,109** (was 1,336 pre-Phase-0a; **+2,773 over the session**) |
| **Aggregate parity** | **29.1%** (was 9.5% baseline; **+19.6pp** via Phase 0a + 1a + 1b + 1c + 2a + 2b + 2c + 2d + 2e + 2f + 2g) |
| Average per-row coverage | 29.3% (was 9.3%) |
...
### 19.2. Where the 29.1% comes from — three contributing layers
...
### 19.4. Phased lift plan — getting from 29.1% to ≥50%

Each phase below states (a) what to do, (b) which in-scope fields it lifts, (c) a **rough** cell-count estimate (not a promise — the harness quantifies after each phase). Aggregate target: from current **4,109 matches → 7,050+** (50% of 14,100). **Phase 3 (multi-product) puts us at 50%.**
...
Phase 1 lift estimate: ~+2,200 matches (Phase 1a + 1b + 1c already booked **+1,814**, putting Phase 1 ahead of the original target). Cumulative actual: **29.1%** (4,109 / 14,100, including Phase 2a + 2b + 2c + 2d + 2e + 2f + 2g). Remaining Phase 1 headroom: G additional date-format coverage (200+ rows currently parser-empty), AZ French/EU prose patterns (FDES not caught), N Country defaults for inferred provenance.
